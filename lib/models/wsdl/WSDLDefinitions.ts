...
export class WSDLDefinitions {
  ...
  static processService(target: any, options: any): void {

    const definitions = this.getWSDLDefinitions(target);
  ...
  static processOperation(target: string, key: string, requestDataType: any, responseDataType: any): void {
    ...
    const requestComplexType = XSDComplexType.getXSDComplexType(requestDataType.prototype);
    ...
    const responseComplexType = XSDComplexType.getXSDComplexType(responseDataType.prototype);
  ...
  static getWSDLDefinitions(target: any, createIfNotExist: boolean = true): WSDLDefinitions {
    const REFLECT_KEY = 'wsdl:definitions';
    let definitions = Reflect.getMetadata(REFLECT_KEY, target);
